...
def calculate_monthly_portfolio_log_return(
    ...
    """
    Calculates the log return for one month.
    Handles both static and dynamic (from RL actions) threshold/outlier_filter.
    """
    cluster_data = current_cluster_data.copy()

    # --- Probability Filtering ---
    if current_prob_data is not None:
        prob_data = current_prob_data.copy()
        prob_data['max_prob'] = prob_data.max(axis=1)
        combined_data = cluster_data.merge(prob_data[['max_prob']], left_index=True, right_index=True, how='left')
        combined_data['max_prob'] = combined_data['max_prob'].fillna(0)
        
        # Determine if any filtering should be applied
        apply_filtering = False
        quantile_thresholds = 0 # Initialize
        if 'action_outlier_filter' in combined_data.columns and combined_data['action_outlier_filter'].notna().any():
            # Dynamic: use the filter value specific to each firm/cluster
            # Ensure action_outlier_filter is treated as a single value for the month
            dynamic_filter_val = combined_data['action_outlier_filter'].dropna().iloc[0]
            quantile_thresholds = combined_data['max_prob'].quantile(dynamic_filter_val)
            apply_filtering = True
        elif outlier_filter is not None:
            # Static: use the single provided outlier_filter value
            quantile_thresholds = combined_data['max_prob'].quantile(outlier_filter)
            apply_filtering = True

        if apply_filtering:
            combined_data['clusters'] = np.where(combined_data['max_prob'] > quantile_thresholds,
                                                 combined_data['clusters'], 0)
            cluster_data = combined_data.drop(columns=['max_prob'])
            if 'action_outlier_filter' in cluster_data.columns:
                 cluster_data = cluster_data.drop(columns=['action_outlier_filter'])
    ...
    # Determine the threshold for position entry.
    # If a static threshold is given, use it. Otherwise, use the dynamic one from the actions.
    if threshold is not None:
        spread_std = cluster_data['spread'].replace(0, np.nan).std()
        if pd.isna(spread_std) or spread_std < 1e-8: spread_std = 1e-8
        position_threshold = spread_std * threshold
    elif 'action_threshold' in cluster_data.columns:
        # For dynamic thresholds, we need to calculate std per group if threshold is std-based
        group_std = cluster_data.groupby('clusters')['spread'].transform('std').replace(0, np.nan).fillna(1e-8)
        position_threshold = group_std * cluster_data['action_threshold']
    else:
        raise ValueError("Either a static 'threshold' must be provided, or 'action_threshold' must be in the data.")

    # Determine positions based on spread sign and magnitude
    cluster_data['inPortfolio'] = cluster_data['spread'].abs() > position_threshold
    ...
    processed_next_returns = processed_next_returns.fillna(-0.5)
